...
name: humanizer-zh
description: |
  Use when writing, rewriting, editing, or reviewing Chinese text, including 口播稿、旁白、解说词、
  短视频文案、文章和报告；也用于用户要求去除 AI 味、人性化、自然、有趣，
  或将语气调整到“不太口语化，也不太书面化”的中间状态。
allowed-tools:
  - Read
  - Write
  - Edit
  - AskUserQuestion
metadata:
  trigger: 写作、修改或审阅中文文本，包括口播稿、旁白、解说词和短视频文案
...
当任务是创作或修改中文口播稿、旁白、解说词或短视频文案时，**必须读取并默认采用 [知识口播默认风格](references/oral-script-style.md)**。用户明确指定的人设、平台、受众、篇幅或语气要求优先；未指定时，使用该默认风格。
