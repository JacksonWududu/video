...
1. Require checksum-valid `narration-audio-source-selection-v1` after Gate 2, the episode-wide white-cat style, density, and approval mode. Accept only `colocated_voice` or `edge_tts`; never infer a source from missing files, one-click mode, or prior episodes.
