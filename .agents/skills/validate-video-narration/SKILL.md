...
6. Validate the first sentence with the opening-text contract by running `python3 scripts/validate_opening_topic.py <candidate-path>` from this Skill directory, record its exact bytes and extracted non-empty topic text, and treat a mismatch as a Gate 2 blocker. Do not insert, repair, shorten, or paraphrase it.
