...
description: "Use when auditing or locking a knowledge-video narration, validating its opening sentence, obtaining Gate 2 approval, selecting colocated voice audio or approved Edge TTS, comparing transcript to script, repairing pauses, locking timing, or validating replacement narration audio for a completed revoice variant. 中文：适用于知识视频口播审计与锁定、首句验证、Gate 2、选择同目录音频或获批的 Edge TTS、转写比对、停顿修复、计时锁定，或已完成项目换音色版本的新旁白验证。"
---
...
## Select and acquire standard-route narration audio

1. Require checksum-valid `narration-audio-source-selection-v1` after Gate 2, the episode-wide white-cat style, density, and approval mode. It may be atomically committed with those choices by the orchestrator's checksum-valid `post-cover-selection-batch-v1`; when present, consume it without asking the source question again. Accept only `colocated_voice` or `edge_tts`; never infer a source from missing files, one-click mode, or prior episodes.
2. For `colocated_voice`, use only the real topic folder already resolved during script lookup. Do not scan sibling or unrelated folders. Inspect only real top-level regular files, reject symbolic links, prefer exact `voice.mp3`, and otherwise require one unique `voice*.mp3`.
3. Record the colocated-file permission and provenance, copy its exact bytes non-destructively into versioned `assets/audio/user-source/`, and verify matching checksums. Never create `<episode-workspace>/audio/`.
4. For `edge_tts`, require the explicit selection to bind provider `edge-tts`, voice `zh-CN-YunjianNeural`, rate `+20%`, locked-script SHA-256, workflow-mode SHA-256, and network authorization. Run `scripts/synthesize_edge_tts.py --text-file <locked-script> --output <versioned-mp3> --metadata-output <versioned-jsonl> --voice zh-CN-YunjianNeural --rate=+20%`; store generated provider audio under `assets/audio/` and its exact provider word-boundary stream under `assets/narration/`, not `assets/audio/user-source/`.
5. Before Edge synthesis, check existing global, bundled, and workspace runtimes for a reusable `edge-tts` installation. If none exists, state what package is needed and why, then obtain explicit installation permission. A missing dependency, network failure, throttling, empty output, or exhausted retry is a blocker; never silently fall back to colocated audio, Voicebox, macOS `say`, or another TTS provider.
6. Preserve the selected source and every generated byte. Derive a new versioned lossless PCM WAV only when downstream safety requires it.
