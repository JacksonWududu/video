---
name: mono-color
description: Generate original one-ink or controlled two-ink editorial images from any theme, sentence, article idea, object, or reference photo. Use this skill for 单色海报、双色印刷、单色调视觉、蓝色/绿色孔版印刷、risograph、网点照片、复古或当代编辑排版、zine poster, monochrome editorial poster, duotone print, or explicit mono-color styling. It uses an adaptive white, gray, or pale-beige substrate, no more than two printing inks, active negative space, terse human language, and strong serif/grotesk/mono typography without making retro styling the default or copying a source composition, wording, logo, or artwork. For knowledge-video episode work, use it only as an approved visual treatment inside the active project workflow; it must not bypass run-knowledge-video, storyboard, visual-production, assembly, approval, transition, or QA gates. Produce both the final generation prompt and the generated raster image unless the user explicitly asks for prompt only.
---
...
Save generated files only under a user-explicit project path or the single resolved root-relative episode workspace. Never create or write `~/Desktop/Claude skills/mono-color/`. When no safe project path or image-generation capability is available, stop at prompt-only and state the limitation.
