...
# ──────────────────────────────────────────────────────────────
# 转码（项目适配版只允许既有系统 ffmpeg）
# ──────────────────────────────────────────────────────────────
def transcode_h264(src: Path, dst: Path) -> Path:
    """
    把 mp4v 原始视频转码为 H.264（yuv420p），提升播放器兼容性。

    临时 mp4v 仅作进程内中间帧容器；交付输出必须由既有系统 ffmpeg
    转为 H.264。缺失或失败即阻断，不安装或调用 PyAV，也不保留中间编码。
    """
    ...
    raise RuntimeError("系统 ffmpeg 不可用；项目适配版禁止 PyAV 或 mp4v 回退")
...
if __name__ == "__main__":
    raise SystemExit("direct upstream CLI disabled; use render_whiteboard_clip.py")
