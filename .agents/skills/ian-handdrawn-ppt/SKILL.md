...
When an approved knowledge-video shot selects `ian-handdrawn-ppt`, use `ian-knowledge-video-frame-v1` instead of the ordinary deck workflow. Read `references/knowledge-video-frame.md` completely. This mode produces exactly one 1920×1080 final PNG per queue item, disables the ordinary page shell, and binds all visible text to the current `per-shot-visual-direction-review-v3` row.
...
- `references/knowledge-video-frame.md`: mandatory v3-bound, one-shot/one-raster adapter for knowledge videos.
...
1. **Ingest material**
   ...
   - If the input is a knowledge-video queue item, stop the ordinary deck branch and validate the inputs required by `ian-knowledge-video-frame-v1` before generation.
...
6. **Build output**
   ...
   - In knowledge-video mode, apply the adapter's prompt override: no automatic page number/title/subtitle/labels/signature, no multi-page expansion, and no text outside the exact approved v3 value. Write and validate the `ian-knowledge-video-frame-v1` manifest before Visual Asset Review.
...
- Knowledge-video exception: one exact 1920×1080 PNG for one queue item, with no ordinary deck shell or inferred text.
