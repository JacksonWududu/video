...
When an approved knowledge-video shot selects `ian-handdrawn-ppt`, use `ian-knowledge-video-layered-scene-v1` instead of the ordinary deck workflow. Read `references/knowledge-video-frame.md` completely. This mode produces one static layered-scene package per queue item: one opaque paper background, ordered full-canvas transparent semantic-element PNGs, and one deterministic final-composite review PNG. Every layer is bound to an exact narration UTF-8 byte range and entry frame; only the fixed layer opacity fade may animate. Whole-scene or per-layer translation, scaling, rotation, masking, and internal cuts are forbidden. All visible text remains bound to the current `per-shot-visual-direction-review-v3` row.
...
1. **Ingest material**
   ...
   - If the input is a knowledge-video queue item, stop the ordinary deck branch and validate the inputs required by `ian-knowledge-video-layered-scene-v1` before generation.
...
6. **Build output**
   ...
   - In knowledge-video mode, apply the adapter's prompt override: no automatic page number/title/subtitle/labels/signature, no multi-page expansion, and no text outside the exact approved v3 value. Generate the background and each semantic element as independent full-canvas assets, not by masking or cropping a complete page. Build the final composite deterministically, then write and validate the `ian-knowledge-video-layered-scene-v1` manifest before Visual Asset Review.
...
- Knowledge-video exception: one exact static layered-scene package for one queue item, with no ordinary deck shell, inferred text, raster transforms, or mask reveals.
