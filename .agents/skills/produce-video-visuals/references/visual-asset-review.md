...
For selected `illustrated-flipbook`, apply `static-spread.md`: Ian and ImageGen each review one complete static image plus checksum-bound prompt, exact narration and half-page readability evidence. Include `static_spread_review` in the existing approval payload; no Ian layer package or cumulative-layer sheet is required for this branch.
...
- Count every distinct non-repairable QA-rejected generated output under `storyboard-image-generation-attempt-limit-v1`, regardless of route. An Ian complete master with intact, mutually separated semantic groups and only `IAN_ZONE_GEOMETRY` or `IAN_ZONE_GEOMETRY_AND_SUBTITLE_SAFE_BAND` is recorded instead under `ian-generation-layout-repair-disposition-v1`, does not increment the rejected count, and pauses new generation for deterministic pre-split repair of those same bytes. All semantic, text, identity, style, lineage, overlap, clipping, missing-group, and non-deterministically-repairable defects remain counted. The third counted rejection for one stable logical asset pauses the full queue and requires user takeover; prompt, reference, base, composition, model, route, path, version, and revision changes never reset the count. Do not generate a fourth automatic attempt or advance to a later queue item. Final package QA and exact-byte Visual Asset Review remain strict.
