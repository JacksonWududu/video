...
Bind every post-opening shot to the validated narration master, active assets, `action-state-schedule-v2` when applicable, exact transition/text/route fields, and its passing `per-shot-visual-direction-review-v3` row. The builder validates catalogs, map hash, route-resolved visible text, Comic prerequisites, exact asset-route equality, and the absence of generic top-title/timeline-text fields. Ordinary `imagegen` with `white_cat_present: true` and every `xuan-paper-diorama` shot must carry `text-free-v1`, `visible_text_mode: none`, null copy/placement, and no title. 宣纸 assets additionally require exact prompt/reference checksums and the catalog-pinned `$generate-visual-styles` Skill/profile fingerprints. Other routes retain their own raster or annotation text policies.

Apply `explicit-visual-generation-route-v3` to new or modified scenes. Route `imagegen` and `xuan-paper-diorama` to `NarrativeScene`, Ian and Ink Doodle Knowledge Card to `GraphicScene`, and Whiteboard to `WhiteboardScene`. 宣纸 and Ink scenes use approved raster bytes directly; no paper overlay, shader, SVG/HTML reconstruction, or route fallback may simulate them. Reject `comic-imagegen` and `doodle-slides` in new, modified, revoice, still, preview, Studio, composition-lock, and render work. Historical unchanged v1/v2 plans remain readable as evidence only; no plan may infer or fall back from a missing route.
...
For every `ink-doodle-knowledge-card` visual, load only the exact-byte-approved `1920×1080` PNG whose checksum and `ink-doodle-knowledge-card-route-v1` evidence are bound in the manifest. Revalidate the route-catalog-pinned `$generate-visual-styles` Skill/profile plus exact prompt/reference/style fingerprints. `GraphicScene` must not redraw or relabel the frame or use `FullFrameMaskSweep`; Ian's mask-sweep rule must never leak into Ink. One state holds its PNG under the approved shot motion/transition contract. Multiple separately approved Ink PNG states use exactly one fixed 0.6-second `intra-shot-watercolor-bloom-v1` transition for each adjacent pair. Historical unchanged `doodle-slides` evidence may be parsed by `DoodleScene` only; it may not produce a new output.
...
For `revoice_variant`, reject a parent using `comic-imagegen`. Otherwise preserve the parent v1/v2/v3 direction/transition evidence, shot/asset/state count and order, bytes, route, text, motion, and every inter-shot transition kind/options/duration seconds/duration frames. Consume only replacement timing and shot/boundary placement; block when a derivative transition differs, a shot is shorter than its locked transition, a redistributed multi-state occurrence leaves the 18–75-frame span, or it has fewer than 15 clean frames after reveal.
...
- Keep character-family camera/crop fixed during each image sequence.
- Validate `action-state-schedule-v2` for applicable character shots: one state is valid; multiple states require full coverage, 18–75-frame occurrence spans, at least 15 clean frames after every incoming 18-frame reveal, at most five states, and exactly `N - 1` fixed 0.6-second/18-frame watercolor transitions. Whiteboard remains exempt. Every ordinary inter-shot boundary uses its approved semantic decision.
- Render every adjacent image pair through `leverage-video/src/shared/watercolor-bloom` for exactly 0.6 seconds, or 18 frames at 30 fps. Preserve its deterministic signature: one impact core, eight irregular color-ink lobes, five paper-fiber tendrils, twelve satellite drops, and no luminous white halo.
- Require each assembly scene to carry ordered `image_sequence` plus exactly `N - 1` consecutive `intra_shot_transitions` when `N >= 2`; one-image scenes carry an empty array. Before any Remotion still, Studio, preview, render, or composition lock, require `node .agents/skills/assemble-video-master/scripts/validate-intra-shot-watercolor-transitions.mjs <assembly-plan.json> <composition-source.tsx>` to pass. A textual promise is not execution evidence.
- Derive new-work state count and occurrence frames only from the shared mechanical action-state contract. Independently require every Comic final state to hold at least 0.5 seconds.
- Require a meaningful change before five seconds for static illustrations and every three to five seconds for long data/explanation scenes.
...
2. Inspect every master/action state, every scheduled state boundary, every `intra-shot-watercolor-bloom-v1` first/middle/final frame, representative initial/final frames for every scene, every ordinary inter-shot transition's first/middle/final frame, and every data reveal state.
