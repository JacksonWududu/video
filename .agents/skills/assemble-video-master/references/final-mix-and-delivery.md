...
- Require one passing `knowledge-video-sound-design-v2`, its immutable policy path/SHA, and exact top-level `knowledge-video-sound-effect-track-v2` projection in every current v3 assembly plan. Verify audible structural coverage at S01 frame 0, every later incoming boundary, and every intra-shot transition including `cut`; complete optional-event analysis; exact selection and rejection evidence; cue-group membership/primary identity; source/derived SHA-256 values; cue/sync frames; roles, intensity, render owners, budget/burst/strong-gap checks, official license observation, active library revision, and canonical map hash. Treat average spacing, long visible gaps, and timbre variety only as reported QA advisories.
- Render each generic cue-group primary only through `SoundEffectTrack` and each Ian cue-group primary only through `IanLayeredScene`; covered members remain audibly represented but are not rendered again. Across both renderers, every cue group has one owner and renders once. Mandatory structure does not consume the duration-scaled optional budget. An incompatible mandatory boundary cue near Ian remains present; only compatible same-moment events may merge. Runtime trim, time stretch, pitch shift, approximate substitution, and unlisted audio are forbidden.
- Start the common SFX bus at `1.12`, with narration gain fixed at `1` and normalization disabled. Before the first full render, run the audio-only preflight from the exact projected cue groups and Ian path. If peak exceeds `-1 dBFS`, lower only the one common SFX bus and rerun preflight; never lower narration or independently alter a cue.
...
- Before the first full render, run `node .agents/skills/assemble-video-master/scripts/preflight-sound-mix.mjs <assembly-plan.json> <preflight-evidence.json>` and require its checksum-current pass at or below `-1 dBFS`; it uses an audio-only null sink and must not render video. For current v3 output, run `node .agents/skills/assemble-video-master/scripts/validate-render-sound-mix.mjs <assembly-plan.json> <preflight-evidence.json> <render.mp4>`. Require one mixed audio stream, exact preflight projection identity, narration gain `1`, disabled normalization, the recorded common SFX bus, no BGM in disabled mode, and measured peak at or below `-1 dBFS`. On overflow, change only `bus_gain_multiplier`, invalidate the plan/preflight/render evidence, rerun preflight, rerender, and repeat.
