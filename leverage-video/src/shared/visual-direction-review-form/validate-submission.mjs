...
import {validateVisualDirectionFormSubmission} from './contract.mjs';
...
export const validateEpisodeSubmissionFile = ({episodeWorkspace, submissionPath}) => {
  ...
  const submission = JSON.parse(fs.readFileSync(path.resolve(submissionPath), 'utf8'));
  return validateVisualDirectionFormSubmission({
    review,
    submission,
    storyboardMarkdown,
    episodeWorkspace,
...
if (isCli) {
  if (process.argv.length !== 4) {
    console.error('usage: node validate-submission.mjs <episode-workspace> <submission.json>');
    process.exit(2);
  }
  try {
    const result = validateEpisodeSubmissionFile({
      episodeWorkspace: process.argv[2],
      submissionPath: process.argv[3],
    });
