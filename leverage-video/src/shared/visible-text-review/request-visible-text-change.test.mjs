...
test('replaces required visible text without changing another shot', () => {
  const input = `# 测试分镜草案 v3
...
- 本地视频源：\`null\`。
`;
  const output = reviseStoryboardVisibleText(input, 'S12', 4, {
    mode: 'required',
    exactVisibleText: '新文字\n第二行',
    visibleTextPlacement: '右侧工具节点',
  });
  assert.match(output, /^# 测试分镜草案 v4/m);
  assert.match(output, /^\| S12 \| 2\.000 \| 乙 \| false \| ian-handdrawn-ppt \| 新文字<br>第二行 \| 原文乙 \|$/m);
  assert.match(output, /## S12\n- 可见文字：\`required\`；\`新文字\n第二行\`；右侧工具节点。\n- 本地视频源/);
  assert.match(output, /## S11\n- 可见文字：\`none\`/);
  assert.doesNotMatch(output, /旧文字/);
});
...
test('policy-authorizes a generic required-copy replacement', () => {
  const policySha256 = 'a'.repeat(64);
  const priorReview = {
    contract_version: 'per-shot-visual-direction-review-v3',
    catalog_version: 'catalog',
  ...
  assert.equal(revised.rows[0].visible_text_mode, 'required');
  assert.equal(revised.rows[0].exact_visible_text, '新文字');
  assert.equal(revised.rows[0].visible_text_placement, '右侧工具节点');
  assert.equal(revised.rows[0].user_selection.exact_visible_text, '新文字');
  assert.equal(revised.status, 'policy_authorized');
  assert.equal(revised.presented_map_sha256, buildPresentedMapSha256(revised));
});
...
test('keeps the revised draft as the active storyboard input for finalization', () => {
  assert.deepEqual(buildRevisedDraftActiveStoryboardBinding({
    path: 'leverage-video/src/episode-test/assets/narration/storyboard-draft-v4.md',
