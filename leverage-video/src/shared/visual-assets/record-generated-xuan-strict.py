...
def record(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("asset is not a strict Xuan Paper master")

    qa_file = resolve_root_relative(args.qa_path, "QA evidence path")
    qa = json.loads(qa_file.read_text(encoding="utf-8"))
    if (
        qa.get("contract_version") != "xuan-paper-diorama-asset-qa-v1"
        ...
        or not qa["actual_reference_inputs"]
    ):
        raise ValueError("Xuan Paper QA evidence is incomplete")

    prompt_file = checksum_bound_file(qa["prompt"], "base prompt")
    ...
    checksum_bound_file(qa["final_correction_prompt"], "final correction prompt")
    reference_evidence_file = checksum_bound_file(
        qa["historical_reference_evidence"], "historical reference evidence"
    )
    reference_evidence = json.loads(reference_evidence_file.read_text(encoding="utf-8"))
    if reference_evidence.get("role") != "identity_and_era_reference":
        raise ValueError("historical identity-and-era reference is missing")
    ...
    for index, stage in enumerate(qa["generation_lineage"]):
        ...
        for reference_index, reference in enumerate(stage.get("reference_inputs", [])):
            checksum_bound_file(reference, f"generation stage {index} reference {reference_index}")
    validate_white_cat_qa(item, qa, normalization["normalized"], normalized_file)
    checks = ["semantic_qa", "visible_text_qa", "style_qa", "visual_qa"]
    if item.get("white_cat_present") is not True:
        checks.append("historical_identity_qa")
    for check in checks:
        if qa.get(check, {}).get("result") != "pass":
            raise ValueError(f"{check} did not pass")
    if qa["visible_text_qa"].get("no_visible_text") is not True or qa["style_qa"].get("readable_depth_planes") not in range(4, 8):
    ...
    item.update(
        ...
        final_correction_prompt_path=qa["final_correction_prompt"]["path"],
        final_correction_prompt_checksum_sha256=qa["final_correction_prompt"]["checksum_sha256"],
        ...
        historical_reference_evidence_path=qa["historical_reference_evidence"]["path"],
        historical_reference_evidence_checksum_sha256=qa["historical_reference_evidence"]["checksum_sha256"],
        ...
        exact_presentation_message=(
            "现呈交 S02 主图精确 PNG，等待用户明确批准此精确字节后，方可生成其动作变体。"
        ),
    )
    if item.get("white_cat_present") is True:
        item["qa_contract_version"] = qa["contract_version"]
        item["identity_qa"] = qa["identity_qa"]
    else:
        item["historical_identity_qa"] = qa["historical_identity_qa"]
    if one_click:
    ...
    temporary = state_file.with_suffix(".json.xuan-strict.tmp")
    if temporary.exists():
        raise ValueError("Xuan Paper strict QA temporary path already exists")
