...
def request_change(args: argparse.Namespace) -> dict:
    ...
    if (
        ...
        or review.get("current_asset_id") != args.asset_id
    ):
        raise ValueError("strict visual review boundary is not active for this asset")
    gate = load_gate()
    item = gate.record_changes_requested(
        state,
        args.asset_id,
        args.decision_message,
        args.decision_time,
    )
    if item.get("strict_review") is not True:
        raise ValueError("asset is not strict-review governed")
    ...
    temporary = state_file.with_suffix(".json.strict-change.tmp")
    if temporary.exists():
        raise ValueError("strict change temporary path already exists")
