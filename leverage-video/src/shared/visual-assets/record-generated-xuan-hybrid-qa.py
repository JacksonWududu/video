...
def is_action_variant(item: dict[str, Any]) -> bool:
...
def record(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("asset is not a non-strict Xuan Paper action state")
    ...
    if (
        ...
        raise ValueError("Xuan Paper action QA evidence is incomplete")
    ...
    for index, stage in enumerate(qa["generation_lineage"]):
        ...
        if references != qa["actual_reference_inputs"]:
    ...
    next_item = next(
        (candidate for candidate in active
         if candidate.get("status") not in {"approved", "qa_passed_pending_batch_review"}),
        None,
    )
    state["visual_asset_review"]["current_asset_id"] = next_item.get("asset_id") if next_item else None

    temporary = state_file.with_suffix(".json.xuan-hybrid-qa.tmp")
    if temporary.exists():
        raise ValueError("Xuan Paper hybrid QA temporary path already exists")
