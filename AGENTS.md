...
- After visual lock, pass automatic sound-design/acquisition; then before episode scripts pass shared-reuse pre-script validation, and before Remotion pass consumption validation. Legacy migration needs exact authorization and unchanged script bytes.
- Approved narration, audio, storyboard, selections, images, timing, caption mode, and delivery roles are immutable contract inputs. Publishing covers remain separate artifacts and must not enter storyboard rows, Remotion timelines, masters, or internal QA roles. Missing, ambiguous, stale, unsupported, unapproved, or substituted inputs block work.
...
- No opening brand/topic Gate. Under `direct-first-shot-v1`, narration and `S01` start at frame 0; `OPEN-00` and fixed-cover prefixes are historical read-only.
